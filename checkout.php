...
if(isset($_POST['order_btn'])){
   ...
   $order_query = mysqli_query($conn, "SELECT * FROM `orders` WHERE nama = '$name' AND no_telp = '$number' AND email = '$email' AND metode = '$metode' AND alamat = '$address' AND total_film = '$total_products' AND total_harga = '$cart_total'") or die('query failed');
   ...
   if($cart_total == 0){
      ...
      if(mysqli_num_rows($order_query) > 0){
         ...
         mysqli_query($conn, "INSERT INTO `orders`(user_id, nama, no_telp, email, metode, alamat, total_film, total_harga, tgl_orders) VALUES('$user_id', '$name', '$number', '$email', '$metode', '$address', '$total_products', '$cart_total', '$placed_on')") or die('query failed');
